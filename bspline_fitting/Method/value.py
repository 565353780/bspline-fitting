...
def basis_functions(degree, knot_vector, spans, knots):
    ...
    for i in range(len(spans)):
        ...
        current_basis_function = basis_function(degree, knot_vector, span, knot)
...
def basis_function_torch(degree, knot_vector, span, knot):
    ...
    N = [torch.ones([1], dtype=dtype).to(device) for _ in range(degree + 1)]
...
def toTorchPoints(
    ...
    basis_u = bs_fit_cpp.basis_functions_torch(degree_u, knotvector_u, spans_u, knots_u)
    basis_v = bs_fit_cpp.basis_functions_torch(degree_v, knotvector_v, spans_v, knots_v)
